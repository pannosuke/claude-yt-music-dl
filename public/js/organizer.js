...
async function handlePlexConnect(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                const libraryPanel = document.getElementById('plexLibraryPanel');
                if (libraryPanel) {
                    scrollToElement(libraryPanel);
                }
            }, 100);
        } else {
            ...
            for (const line of lines) {
                if (line.startsWith('data: ')) {
                    ...
                    if (data.completed && data.results) {
                        ...
                        setTimeout(() => {
                            const comparisonPanel = document.getElementById('comparisonResults');
                            if (comparisonPanel) {
...
async function handleExecuteArtistRenames() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Auto-scroll to Phase 2 button
            setTimeout(() => {
                scrollToElement(matchAlbumsBtn);
            }, 100);
        } else {
